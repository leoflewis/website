...
export class PlayerCardComponent {
  ...
  values: number[];
  colors: string[];
  labels = ["Games", "EV TOI","Points", "Goals", "Assists", "Shots", "Blocks", "EVTOI", "FO %", "GWG", "Hits", "OTG", "PIMs", "+/-", "PP G", "PP P", "PP TOI", "SH G", "SH P", "SH TOI", "Shifts"];
  label = "Z-Score";
  ...
  ngOnInit() {
    if (this.id != undefined){
      this.getPlayerData(this.id).subscribe(data => {
        ...
        this.values.push(this.playerData.message.data.EVTOI);
        this.values.push(this.playerData.message.data.FOpct);
